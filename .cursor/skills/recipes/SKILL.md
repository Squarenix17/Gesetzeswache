...
- `guide` — lists commands and where to read docs, but as a flat catalog.
...
- User wants deep docs for ONE command — use `guide`.
...
```bash
for D in \
  "$HOME"/.cursor/plugins/marketplaces/ecc/commands \
  "$HOME"/.cursor/plugins/cache/ecc/ecc/*/commands \
  ./commands \
  ./.cursor/commands \
  "$HOME"/.cursor/commands; do
  [ -d "$D" ] && CMD_DIR="$D" && break
done
...
5. **Where to read** — the `commands/<name>.md` path plus `/guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /guide <cmd1>)
```
...
**Match:** `/recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `guide`.
