---
name: config-gc
description: Garbage collection for your Cursor configuration. Periodically scans ~/.cursor (skills, memory, hooks, permissions, MCP servers, caches) for redundant, stale, orphaned, or low-value items, then walks the user through a confirm-each-deletion cleanup. Use when the user says "clean up my config", "config GC", "too many skills", "audit my setup", "my .cursor is bloated", or asks for a periodic config review.
metadata:
  origin: ECC
---
...
| 5 | MCP servers | `~/.cursor/cli-config.json` or project `.mcp.json` | Servers that fail to connect; functional duplicates; long-unused |
...
- **Touching anything outside `~/.cursor`** (or the project's `.cursor/`). Config GC never wanders into source trees.
