...
function findPluginInstall(rootDir) {
  ...
  const pluginRoots = uniquePaths([
    path.join(rootDir, '.cursor', 'plugins'),
    ...homeDirs.map(homeDir => path.join(homeDir, '.cursor', 'plugins')),
  ]);
  const installedPluginsPaths = uniquePaths([
    path.join(rootDir, '.cursor', 'plugins', 'installed_plugins.json'),
    ...homeDirs.map(homeDir => path.join(homeDir, '.cursor', 'plugins', 'installed_plugins.json')),
  ]);
...
function getConsumerChecks(rootDir) {
  ...
  const projectHooks = safeRead(rootDir, '.cursor/settings.json');
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.cursor/plugins/ecc/ (legacy everything-claude-code paths also supported)',
      description: 'Everything Cursor is installed for the active user or project',
    ...
    },
    {
      ...
      path: '.cursor/',
      description: 'Project-specific harness overrides exist under .cursor/',
      pass: countFiles(rootDir, '.cursor/agents', '.md') > 0 ||
        countFiles(rootDir, '.cursor/skills', 'SKILL.md') > 0 ||
        countFiles(rootDir, '.cursor/commands', '.md') > 0 ||
        fileExists(rootDir, '.cursor/settings.json') ||
        fileExists(rootDir, '.cursor/hooks.json'),
      fix: 'Add project-local .cursor hooks, commands, skills, or settings that tailor ECC to this repo.',
    },
    {
      ...
      pass: fileExists(rootDir, 'AGENTS.md') || fileExists(rootDir, 'AGENTS.md') || fileExists(rootDir, '.cursor/AGENTS.md'),
      fix: 'Add AGENTS.md or AGENTS.md so the harness has project-specific instructions.',
    },
    {
      ...
      pass: fileExists(rootDir, '.mcp.json') || fileExists(rootDir, '.cursor/settings.json') || fileExists(rootDir, '.cursor/settings.local.json'),
      fix: 'Add .mcp.json or .cursor/settings.json so project-local tool configuration is explicit.',
    },
    {
    ...
    },
    {
      ...
      points: 2,
      scopes: ['repo'],
      path: '.cursor/memory.md',
      description: 'Project memory or durable notes are checked in',
      pass: fileExists(rootDir, '.cursor/memory.md') || countFiles(rootDir, 'docs/adr', '.md') > 0,
    ...
    },
    {
      ...
      pass: projectHooks.includes('PreToolUse') || projectHooks.includes('beforeSubmitPrompt') || fileExists(rootDir, '.cursor/hooks.json'),
