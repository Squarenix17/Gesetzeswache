 ...
 *
 * Reads bridge file from metrics-bridge.js and injects agent-facing
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
