...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `metrics-bridge` and the background
 * `statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/metrics-bridge.js` (commit 9b1d8918) for the
