 ...
 *
 * Maintains a running session aggregate in /tmp/metrics-{session}.json.
 * This bridge file is read by statusline.js and context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
