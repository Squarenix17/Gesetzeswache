 ...
 *
 * Tries, in order:
 *   1. CLAUDE_PLUGIN_ROOT env var (set by Cursor for hooks, or by user)
 *   2. Standard install location (~/.cursor/) — when scripts exist there
 *   3. Known plugin roots under ~/.cursor/plugins/ (current + legacy slugs)
 *   4. Plugin cache auto-detection — scans ~/.cursor/plugins/cache/{ecc,everything-claude-code}/
 *   5. Fallback to ~/.cursor/ (original behaviour)
 *
...
function resolveEccRoot(options = {}) {
  ...
  // Standard install — files are copied directly into ~/.cursor/
  ...
  // Plugin cache — Cursor stores marketplace plugins under
  // ~/.cursor/plugins/cache/<plugin-name>/<org>/<version>/
  try {
 ...
 * When CLAUDE_PLUGIN_ROOT is set (as Cursor does for plugin hooks and
 ...
 * under ~/.cursor/plugins/, and the versioned plugin cache — only far enough to
