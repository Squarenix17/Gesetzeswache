...
const STRUCTURED_DIRS = /(^|\/)(docs|\.cursor|\.github|commands|skills|benchmarks|templates|\.history|memory)\//;
  ...
  if (filePath && isSuspiciousDocPath(filePath)) {
    return {
      ...
      additionalContext: [
        ...
        `[Hook] File: ${filePath}`,
        '[Hook] Consider using a structured path (e.g. docs/, .cursor/, skills/, .github/, benchmarks/, templates/)',
      ],
    };
  }
