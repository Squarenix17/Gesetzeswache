...
# Runs on Stop hook to extract reusable patterns from Cursor sessions
#
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.cursor/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (Cursor hook input)
