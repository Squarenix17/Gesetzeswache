---
name: agentic-os
description: Build persistent multi-agent operating systems on Cursor. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases.
metadata:
  origin: ECC
---
...
├── .cursor/commands/  # Slash commands: user-facing CLI
...
| Commands (`.cursor/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`AGENTS.md` is the kernel. It acts as the COO / orchestrator. Claude reads it at session start and uses it to route work.
...
- Test runner access
- MCP servers as configured in `.cursor/mcp.json`
...
Slash commands are markdown files in `.cursor/commands/`. They define reusable workflows.
...
Place command files in `.cursor/commands/<command-name>.md`. Cursor auto-discovers them. Users invoke them with `/<command-name>`.
