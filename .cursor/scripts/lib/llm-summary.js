 ...
 *
 * Uses `claude -p` (Cursor CLI) to generate rich, contextual session
 * summaries from JSONL transcripts. Requires no API key — reuses Cursor's
 * own authentication.
 *
...
function generateSessionSummary(transcriptPath) {
  ...
  const prompt = [
    'Below is a conversation log from a Cursor coding session.',
