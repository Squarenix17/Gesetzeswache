...
- You just wrote a plan artifact (`.cursor/plans/*.plan.md` from `/plan`) and
...
ecc-plan-canvas open .cursor/plans/feature.plan.md
...
ecc-plan-canvas await .cursor/plans/feature.plan.md
```
...
`.cursor/plans/notifications.plan.md` and must WAIT for confirmation:

```bash
ecc-plan-canvas open .cursor/plans/notifications.plan.md
ecc-plan-canvas await .cursor/plans/notifications.plan.md
# → {"status":"feedback","items":[{"kind":"verdict","verdict":"approve"}]}
ecc-plan-canvas end .cursor/plans/notifications.plan.md
