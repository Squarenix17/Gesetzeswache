---
name: configure-ecc
description: Interactive installer for Everything Cursor — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
metadata:
  origin: ECC
---

# Configure Everything Cursor (ECC)

An interactive, step-by-step installation wizard for the Everything Cursor project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup everything cursor", or similar
...
This skill must be accessible to Cursor before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install ecc@ecc` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.cursor/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (~/.cursor/)" — "Applies to all your Cursor projects"
  - "Project-level (.cursor/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.cursor`
- Project-level: `TARGET=.cursor` (relative to current project root)
- Both: `TARGET_USER=~/.cursor`, `TARGET_PROJECT=.cursor`
...
```bash
grep -rn "~/.cursor/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.cursor/` paths:
- If a skill references `~/.cursor/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.cursor/skills/` or `~/.cursor/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.cursor/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.cursor/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .cursor/skills/")
...
### "Skills not being picked up by Cursor"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.cursor/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.cursor/skills/<skill-name>/SKILL.md` exists
...
- Some skills assume `~/.cursor/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.cursor/homunculus/` directory is always user-level — this is expected and not an error.
