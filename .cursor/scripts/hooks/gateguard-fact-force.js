...
function isHarnessSettingsPath(filePath) {
  const normalized = normalizeForMatch(filePath);
  return /(^|\/)\.cursor\/settings(?:\.[^/]+)?\.json$/.test(normalized);
}
...
function run(rawInput) {
  ...
  if (toolName === 'Edit' || toolName === 'Write') {
    ...
    if (!filePath || isHarnessSettingsPath(filePath) || isExemptPath(filePath)) {
  ...
  if (toolName === 'MultiEdit') {
    ...
    for (const edit of edits) {
      ...
      if (filePath && !isHarnessSettingsPath(filePath) && !isExemptPath(filePath) && !isChecked(filePath)) {
        const { ok, denials } = markCheckedAndCountDenial(filePath);
        if (!ok) {
          return allowWithStateWarning();
        }
