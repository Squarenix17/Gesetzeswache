 ...
 * body. See issue #2107.
 *
 * The Cursor surface already routes through the local, in-repo hook
 ...
 * to the legacy hook `tool_input.command` shape the local hook understands,
...
readStdin()
  .then(raw => {
    ...
    // Local hook accepts either the raw command string or a legacy hook
    // shaped `{ tool_input: { command } }` JSON. Pass the legacy shape so
    // the JSON branch in extractCommand() is exercised the same way the
    // legacy hook surface exercises it — keeps the two surfaces on the
    // same code path.
    const legacyInput = JSON.stringify({ tool_input: { command } });
    const result = run(legacyInput);
