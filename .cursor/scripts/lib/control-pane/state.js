...
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.cursor', 'ecc', 'state.db');
...
function defaultDbPath(env = process.env) {
  return path.join(homeDir(env), '.cursor', 'ecc2.db');
}
...
function defaultConfigPaths(cwd = process.cwd(), env = process.env) {
  ...
  const paths = [path.join(home, 'Library', 'Application Support', 'ecc2', 'config.toml'), path.join(home, '.config', 'ecc2', 'config.toml'), path.join(home, '.cursor', 'ecc2.toml')];

  let current = path.resolve(cwd);
  while (current && current !== path.dirname(current)) {
    paths.push(path.join(current, '.cursor', 'ecc2.toml'));
