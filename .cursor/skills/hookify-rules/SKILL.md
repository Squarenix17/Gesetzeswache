...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.cursor/hookify.{rule-name}.local.md` files.
...
- **Naming**: `.cursor/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.cursor/*.local.md` to `.gitignore`
