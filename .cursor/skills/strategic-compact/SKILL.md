...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Edit",
        "hooks": [{ "type": "command", "command": "node ~/.cursor/scripts/hooks/suggest-compact.js" }]
      },
      {
        "matcher": "Write",
        "hooks": [{ "type": "command", "command": "node ~/.cursor/scripts/hooks/suggest-compact.js" }]
...
- `CLAUDE_CODE_AUTO_COMPACT_WINDOW` — Cursor's native window-size override, in tokens; honored as a fallback when `ECC_CONTEXT_WINDOW_TOKENS` is unset.
...
| Memory files (`~/.cursor/memory/`) | Multi-step conversation context |
...
- Same rules in both `~/.cursor/rules/` and project `.cursor/rules/`
