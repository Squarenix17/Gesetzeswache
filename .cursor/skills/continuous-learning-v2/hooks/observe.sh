...
# Cursor passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.cursor/settings.json.

set -e

# Hook phase from CLI argument: "pre" (PreToolUse) or "post" (PostToolUse).
# Manual settings.json installs can call this script without the plugin
# wrapper's positional phase argument, but Cursor still exposes the hook
...
# Read JSON from stdin (Cursor hook format)
INPUT_JSON=$(cat)
...
_ECC_SKIP_PATHS="${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.cursor-mem}"
if [ -n "$STDIN_CWD" ]; then
...
# Pass HOOK_PHASE via env var since Cursor does not include hook type in stdin JSON
...
try:
    ...
    # Cursor does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - Cursor hook format
