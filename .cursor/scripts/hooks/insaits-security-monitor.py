#!/usr/bin/env python3
"""
InsAIts Security Monitor -- PreToolUse Hook for Cursor
============================================================

Real-time security monitoring for Cursor tool inputs.
...
How it works:
  Cursor passes tool input as JSON on stdin.
...
def extract_content(data: Dict[str, Any]) -> Tuple[str, str]:
    """Extract inspectable text from a Cursor tool input payload.
...
def format_feedback(anomalies: List[Any]) -> str:
    """Format detected anomalies as feedback for Cursor.
...
def main() -> None:
    """Entry point for the Cursor PreToolUse hook."""
    ...
    if has_critical:
        # stdout feedback -> Cursor shows to the model
