...
No install command, no plugin config. The hooks in `.cursor/settings.json` are picked up automatically when you run Cursor in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.cursor/hooks/` directory to your project
2. Copy `.cursor/settings.json` hook configuration
...
Plankton's `.cursor/hooks/config.json` controls all behavior:
